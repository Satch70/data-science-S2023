...
The following is a *unit test* of your code; if you managed to solve task **q2** correctly, the following code will execute without error.
...
This dataset is called `nycflights`; in what sense is it focused on New York city? Let's do a quick check to get an idea:
...
df_q5b <- flights %>% filter(origin == "JFK" | origin == "LGA" | origin == 'EWR')
df_q5a
df_q5b
```
