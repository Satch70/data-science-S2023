...
- There are more inputs then outputs
- there is spatial and metadata
- many of these numbers are very large
...
tibble(
  err_train_x = rsquare(fit_baseline, df_train),
  err_val_x = rsquare(fit_baseline, df_validate),
  err_train_avg_T = rsquare(fit_cheat, df_train),
  err_val__avg_T = rsquare(fit_cheat, df_validate),
  err_train_idx = rsquare(fit_nonphysical, df_train),
  err_val_idx = rsquare(fit_nonphysical, df_validate),
)
```

    ## # A tibble: 1 × 6
    ##   err_train_x err_val_x err_train_avg_T err_val__avg_T err_train_idx err_val_idx
...
- Which model is *most accurate*? Which is *least accurate*?
  - fit_cheat model is the most accurate.
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - avg_T is so great at predicting t_norm because it is in the equation
    of T_norm and the other inputs aren’t in the equation.
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - We would not therefor avg_T is not a valid predictor.
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - It is the experiment number and it only has meta meaning.
...
fit_q4 <- 
  lm(data = df_train, formula = T_norm ~ x + L + W + T_f + rho_f + U_0  )
...
```
    ...
    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric =
    ## parametric, : reciprocal condition number 2.372e-16
...
fit_q6 <- 
  lm(data = df_train, formula = T_norm ~ x + L + W + U_0  )
...
```
    ...
    ##  9  0.25     9 0.184 0.0419  2.58 2.18e6 0.107   322. 1.07  1.56e-5 0.0292 1186.
    ## 10  0.25    10 0.147 0.0336  1.79 1.68e6 0.0942  333. 1.04  1.80e-5 0.0280  826.
    ## # … with 70 more rows, and 16 more variables: rho_p <dbl>, d_p <dbl>,
    ## #   C_pv <dbl>, h <dbl>, I_0 <dbl>, eps_p <dbl>, avg_q <dbl>, avg_T <dbl>,
    ## #   rms_T <dbl>, T_norm <dbl>, ci_fit <dbl>, ci_lwr <dbl>, ci_upr <dbl>,
    ## #   pi_fit <dbl>, pi_lwr <dbl>, pi_upr <dbl>
...
```
    ...
    ## # A tibble: 1 × 10
    ##       x     L     W   U_0 ci_fit ci_lwr ci_upr pi_fit pi_lwr pi_upr
    ##   <dbl> <dbl> <dbl> <dbl>  <dbl>  <dbl>  <dbl>  <dbl>  <dbl>  <dbl>
    ## 1     1   0.2  0.04     1   1.88   1.58   2.17   1.88   1.23   2.52
...
df_validate <- df_validate %>%
  add_uncertainties(fit_q6, interval = "prediction", prefix = "pi") %>%
  mutate(within_interval = ifelse(T_norm >= pi_lwr & T_norm <= pi_upr, TRUE, FALSE))

# Count the number of points that fall within the prediction interval
num_within_interval <- sum(df_validate$within_interval)

# Calculate the fraction of points that fall within the prediction interval
fraction_within_interval <- num_within_interval / nrow(df_validate)
fraction_within_interval
```
...
```
    ...
    ## # … with 50 more rows, and 14 more variables: rho_p <dbl>, d_p <dbl>,
    ## #   C_pv <dbl>, h <dbl>, I_0 <dbl>, eps_p <dbl>, avg_q <dbl>, avg_T <dbl>,
