...
- What kinds of information *do we not have* about these students?
  - How many classes they took, economic background, extracurricular,
    name, personal background, physical attributes, personal
    preferences, issues.
- Based on these missing variables, what possible effects could be
  present in the data that we would have *no way of detecting*?
  - Why or how they got the grades they got, what other priorities or
    other external factors could affect their scores.
...
- How do the results here compare with the visual you created in q2?
  - Visually there seems to be a correlation that supports the results.
...
- What is the confidence interval on the coefficient of `both_SAT`? Is
  ...
  - the CI is 0.0026 but this is very close to zero, making me assume
    that it is not the most accurate CI.
- By itself, how well does `both_SAT` predict `univ_GPA`?
  - It can give a general idea that with a higher SAT one will have a
    higher GPA but it is not a very accurate correlation.
...
- What is the confidence interval on the coefficient of `both_SAT` when
  ...
  - It is weird that zero falls within the confidence interval. Meaning
    that it doesn’t necessarily do anything. Looking at the difference I
    can see that sat also contains zero, which makes me assume that
    high_GPA is the reliable part.
- How do the hypothesis test results compare with the results in q6?
  - Overall the ability to predict the university gpa is better, but the
    confidence interval has zero within its bounds, making the
...
- Between `both_SAT` and `high_GPA`, which single variable would you
  ...
  - I would choose high_GPA for three reasons. One is that visually the
    trend of high_GPA seems to be more of a linear indicator for
    univ_GPA. Two is when comparing high_GPA ability to predict and the
    validation data, the error was less then when comparing the SAT
    ability to predict. Three, the SAT confidence interval contained
    zero and high_GPA didn’t.
- Is `both_SAT` an effective predictor of `univ_GPA`? What specific
  pieces of evidence do you have in favor of `both_SAT` being effective?
  What specific pieces of evidence do you have against?
  - While there was some error when predicting, it still did a
    relatively good job of predicting. The specific avergae error was
    pretty low when determining the predicted univ_GPA. We can
    definitely use that model to say with a higher SAT you will probably
    have a higher univ_GPA. You can also see this visually confirmed in
    the graph. The piece I think is the biggest specific example of it
    not being a good predictor is the confidence interval containing
    zero.
